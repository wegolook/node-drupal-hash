...
function _password_crypt(algo, password, setting) {
  ...
  const count_log2 = _password_get_count_log2(setting);
  ...
  let hashed = hash(algo, Buffer.concat([salt, password]));
  do {
    hashed = hash(algo, Buffer.concat([hashed, password]));
  } while (--count);

  const len = hashed.length;
  const output = setting + _password_base64_encode(hashed, len);
  // _password_base64_encode() of a 16 byte MD5 will always be 22 characters.
  // _password_base64_encode() of a 64 byte sha512 will always be 86 characters.
  const expected = 12 + Math.ceil((8 * len) / 6);
...
function user_check_password(password, hashedPassword) {
  ...
  const type = stored_hash.substr(0, 3);
  let hashed = null;
  switch (type) {
    case '$S$':
  ...
  if (typeof hashed === 'string') {
    let mismatch = hashed.length === stored_hash.length ? 0 : 1;
    for (let i = 0, l = hashed.length; i < l; ++i) {
...
function user_needs_new_hash(hashedPassword) {
  ...
  if (
    hashedPassword.substring(0, 3) != '$S$' ||
    hashedPassword.length != HASH_LENGTH
  ) {
    return true;
  }
  // Ensure that $count_log2 is within set bounds.
  const count_log2 = _password_enforce_log2_boundaries(HASH_COUNT);
